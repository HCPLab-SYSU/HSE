...
class ResNetEmbed(nn.Module):
    def __init__(self, cdict={}):
        ...
        self.branch_L1 = self._make_branch(level='order')
        self.fc_L1 = nn.Linear(2048, self.num_classes['order'])

        # III.  define the L2(family) branch
        self.branch_L2_guide = self._make_branch(level='family') # fed to guide model
        self.branch_L2_raw = self._make_branch(level='family') # fed to fc
        ...
        self.branch_L4_guide = self._make_branch(level='class') # fed to guide model
        self.branch_L4_raw = self._make_branch(level='class') # fed to fc
